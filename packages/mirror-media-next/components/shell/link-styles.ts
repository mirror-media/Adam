...
const shellTextLinkClass = `${bracket} rounded-mm-xs outline-none transition-colors hover:text-mm-neutral-0 ${focusRing} focus-visible:outline-mm-second-400`

/** Brand lockups keep their colours, so on a light row they dim instead. */
const shellBrandLinkClass = `rounded-mm-xs outline-none transition-opacity hover:opacity-80 ${focusRing} focus-visible:outline-mm-second-500`

/** Lockups on a dark row take the same brackets; their own colours stay put. */
const shellBrandLinkOnDarkClass = `${bracket} outline-none ${focusRing} focus-visible:outline-mm-second-400`

/** Social icons take no brackets; they simply lift toward white. */
const shellIconLinkOnDarkClass = `outline-none transition hover:brightness-150 ${focusRing} focus-visible:outline-mm-second-400`
